...
class Lib_general {
		...
		function getNegeriUser($id_pengguna)
		{
			$CI =& get_instance();	
			
			$query = $CI->db->query("SELECT
			adm_negeri.negeri as negeri
			FROM
			adm_pengguna join adm_negeri on adm_pengguna.id_adm_negeri=adm_negeri.id_adm_negeri
			where adm_pengguna.id_pengguna='$id_pengguna'");
			$query->num_rows();						
			
			if ($query->num_rows() == 1) {
				foreach($query->result() as $row) {
					return $negeri = $row->negeri;	
		...
		function getDaerahUser($id_pengguna)
		{
			$CI =& get_instance();	
			
			$query = $CI->db->query("SELECT
			adm_daerah.daerah as daerah
			FROM
			adm_pengguna join adm_daerah on adm_pengguna.id_adm_daerah=adm_daerah.id_adm_daerah
			where adm_pengguna.id_pengguna='$id_pengguna'");
			$query->num_rows();						
			
			if ($query->num_rows() == 1) {
				foreach($query->result() as $row) {
					return $daerah = $row->daerah;	
		...
		function getPerananUser($id_pengguna)
		{
			$CI =& get_instance();	
			
			$query = $CI->db->query("SELECT
			adm_peranan_rela.nama_peranan_rela as peranan			
			FROM
			adm_pengguna
			INNER JOIN adm_peranan_rela ON adm_pengguna.id_adm_peranan_rela = adm_peranan_rela.id_adm_peranan_rela
			where adm_pengguna.id_pengguna='$id_pengguna'");
			$query->num_rows();						
			
			if ($query->num_rows() == 1) {
				foreach($query->result() as $row) {
					return $peranan = $row->peranan;	
