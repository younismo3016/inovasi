...
(SELECT COUNT(*) FROM ptj WHERE negeri = 'SABAH')as jumlah_negeri,
...
";
		...
		$q .= " SELECT bulan_kkwt as bulan,  
(SELECT COUNT(*) FROM ptj WHERE negeri = 'SARAWAK')as jumlah_negeri,
(select count(*) from ptj p1 where month(cipta_pada) <= b.bulan and status_aktif = 1 and YEAR(cipta_pada) <= $tahun  )  as jumlah_ptj,
SUM(
  CASE WHEN DAY(p.tarikh_terima) >= 1 and DAY(p.tarikh_terima) <=10 and MONTH(p.tarikh_terima) = (b.bulan + 1) and year(p.tarikh_terima) <= $tahun 
  THEN 1
...
";
		...
		$q .= "SELECT bulan_kkwt as bulan, 
(SELECT COUNT(*) FROM ptj WHERE negeri NOT IN('SABAH','SARAWAK'))as jumlah_negeri,		
(select count(*) from ptj p1 where month(cipta_pada) <= b.bulan and status_aktif = 1 and YEAR(cipta_pada) <= $tahun  )  as jumlah_ptj,
SUM(
  CASE WHEN DAY(p.tarikh_terima) >= 1 and DAY(p.tarikh_terima) <=10 and MONTH(p.tarikh_terima) = (b.bulan + 1) and year(p.tarikh_terima) <= $tahun 
  THEN 1
...
group by bulan_kkwt order by id_adm_bulan ASC  ";	
	...
	$q .= "  SELECT bulan_kkwt as bulan,  
(select count(*) from ptj p1 where month(cipta_pada) <= b.bulan and status_aktif = 1 and YEAR(cipta_pada) <= $terkini  )  as jumlah_negeri,
SUM(
...
SUM(
  ...
  ELSE 0
  END ) AS lebih_tempoh, 
((select count(*) from ptj p1 where month(cipta_pada) <= b.bulan and status_aktif = 1 and YEAR(cipta_pada) <= $terkini    ) - COUNT( * ) ) as belum_terima,
COUNT( * ) AS TOTAL
FROM penerimaan p 
INNER JOIN adm_bulan b on p.bulan_kkwt = b.bulan_penuh
		...
		if(isset($_POST['submit']))
		{
			...
			//$B='';
			 foreach($q->result() as $row){
				...
				//echo "-->".$B;
				$kod_ptj = $row->kod_ptj;
				$nama_penyemak = $row->nama_penyemak;
