...
class Main extends CI_Controller {
	...
	function login_process()
	{
		...
		if($user == TRUE){
			...
			$data = array('sess_nama'=> $sess_nama,'sess_level'=>$level,'sess_id'=> $id,'logged_in' => TRUE );
...
{	
	...
	function surat_pertanyaan_todo($id)
	{
		...
		$query = $this->db->query("SELECT nama_ptj,id_ptj FROM ptj where id_ptj='$id'");					
			...
			$query3 = $this->db->query("SELECT * FROM  senarai_kuiri join kuiri1 on kuiri1.id_q1=senarai_kuiri.id_q1 join kuiri2 on kuiri2.id_q2=senarai_kuiri.id_q2 where senarai_kuiri.id_ptj='$id'");					
		...
		$this->pagination->initialize($pagination);
		$data['list'] = $this->model_pengguna->surat_pertanyaan_todo($pagination['per_page'],$this->uri->segment(4,0),$data['id_ptj']);
	...
	$data = array (		
		...
		//$this->output->enable_profiler(TRUE);
		redirect ('main/surat_pertanyaan_todo/'.$id_ptj,'refresh');	
	...
	//Untuk Carian==kena sama dengan model pengguna
		if(isset($_POST['submit']))
		{
			...
			$this->session->set_userdata('sess_bulan_kkwt',$data['bulan_kkwt_kuiri']);
		...
		$this->pagination->initialize($pagination);
		$data['list'] = $this->model_pengguna->maklumbalas_kuiri($pagination['per_page'],$this->uri->segment(4,0),$data['kod_ptj'],$data['bulan_kkwt_kuiri']);
		//papar mesej bila berjaya 
		$data['flash_success'] = $this->session->flashdata('flash_success');	
	...
	//Untuk Carian==kena sama dengan model pengguna
		if(isset($_POST['submit']))
		{
			...
			$q= "SELECT
				...
				FROM
				ptj
				INNER JOIN `user` ON ptj.cipta_oleh = `user`.id_user ";
...
(select count(pn1.id_penerimaan) from penerimaan pn1 where month(tarikh_terima) = b.bulan AND pn1.tarikh_terima BETWEEN '".$tempoh_laporan1."' AND '".$tempoh_laporan2."' ) as jumlah_diterima
from adm_bulan b join ptj p 
...
(select count(pn1.id_penerimaan) from penerimaan pn1 where month(tarikh_terima) = b.bulan AND pn1.tarikh_terima BETWEEN '".$now."' AND '".$now."' ) as jumlah_diterima
from adm_bulan b join ptj p
...
if($data['zon'] !='Pilih' && $data['nama_penuh'] !='Pilih'){
	...
	$q .= "and p.cipta_oleh = '".$nama_penuh."'";
	
		
}else if($data['nama_penuh'] !='Pilih'){
	
	    $nama_penuh=$data['nama_penuh'];
		$q .= "where p.cipta_oleh = '".$nama_penuh."'";	
...
ORDER BY b.bulan";
		...
		$this->load->view('penerimaan_zon',$data);
		$this->output->enable_profiler(TRUE);
	}
...
{	
	//Untuk Carian==kena sama dengan model pengguna
		...
		$this->pagination->initialize($pagination);
		$data['list'] = $this->model_pengguna->cetak_surat_peringatan($pagination['per_page'],$this->uri->segment(4,0),$data['kod_ptj'],$data['bulan_kkwt']);
		//papar mesej bila berjaya 
		$data['flash_success'] = $this->session->flashdata('flash_success');	
